	...
	tdnCalendar.fn = tdnCalendar.prototype = {
		...
		drawing: function(iYear,iMonth){
			var oWarp = document.createElement("div"),
				...
				dayLi,dayWeek,arr1 =[],arrDate =[],i,j,dateTag,holidayTag,detailTag,tempStr,tsy,tsm,tsd;
			...
			for(i=0;i<all;i++){
				...
				if(arr1[i]==-1){//上个月
					...
					if(this.opt.minDate&&tdnCalendar.checkDate(this.opt.minDate)){
						tempStr = this.opt.minDate;
						tsy = tempStr.substring(0,tempStr.indexOf("-"));
						tsm = tempStr.substring(tempStr.indexOf("-")+1,tempStr.lastIndexOf("-"));
						tsd = tempStr.substring(tempStr.lastIndexOf("-")+1);
						//console.log(tsy,tsm,tsd);
						(iYear<tsy)&&(dayLi.className = "tdn-day-out");
						(tsy==iYear)&&(iMonth<tsm)&&(dayLi.className = "tdn-day-out");
						(tsy==iYear)&&(tsm==iMonth)&&(arrDate[i]<tsd)&&(dayLi.className = "tdn-day-out");/**/
					}
					if(this.opt.maxDate&&tdnCalendar.checkDate(this.opt.maxDate)){
						tempStr = this.opt.maxDate;
						tsy = tempStr.substring(0,tempStr.indexOf("-"));
						tsm = tempStr.substring(tempStr.indexOf("-")+1,tempStr.lastIndexOf("-"));
						tsd = tempStr.substring(tempStr.lastIndexOf("-")+1);
						(iYear>tsy)&&(dayLi.className = "tdn-day-out");
						(iYear==tsy)&&(iMonth>tsm)&&(dayLi.className = "tdn-day-out");
						(iYear==tsy)&&(iMonth==tsm)&&(arrDate[i]>tsd)&&(dayLi.className = "tdn-day-out");
					}
		...
		choiceYear: function( o ){
			...
			if(obj.innerHTML.length==0){
				for(var i=this.opt.minYear;i<=this.opt.maxYear;i++){
